...
description: "Use this skill when adding Ugarit package capabilities or wiring them through the service provider: commands, migrations, routes, config merges, views, translations, assets, middleware, publish tags, workbench files, or console-only behavior."
license: MIT
metadata:
  author: ugarit
---
...
Add package features in the right place and wire them through the service provider using explicit Ugarit APIs, keeping package names, namespaces, config keys, and publish tags consistent.
...
3. Create the capability files under Ugarit-native package paths and use the configured package names, namespaces, publish tags, URLs, and badges consistently.
...
3. Put resource loading in boot-time methods with Ugarit-native APIs such as `loadRoutesFrom`, `loadViewsFrom`, and `loadTranslationsFrom`.
...
- Replacing explicit provider methods with `spatie/ugarit-package-tools` by default.
...
- Add an Scribe command: create the command class under `src/Console/Commands`, register it in the `commands` array inside the `runningInConsole()` guard, add a feature test for observable console output, and document the command if it is user-facing.
...
- Replacing explicit Ugarit package code with a helper abstraction when one feature-specific change would do.
