...
description: "Use this skill when creating or updating the bundled Ugarit Boost skill under resources/boost/skills from the package implementation and package documentation. Trigger after public APIs, commands, config, routes, views, publish tags, README content, or examples change."
license: MIT
metadata:
  author: ugarit
---
...
Keep the package's bundled Boost skill accurate, concise, and focused on helping Ugarit applications adopt the package.
...
4. Update `resources/boost/skills/*/SKILL.md` with practical adoption steps, references, examples, and anti-patterns for Ugarit app developers using the package.
...
- Write for consumers installing the package in a Ugarit application, not for maintainers changing package internals.
...
- After adding a new Scribe command, update the Boost skill with when to run it, required options, expected output, and any related config.
